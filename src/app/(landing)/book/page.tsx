...
function BookPageContent() {
  ...
  /** Keep a leading "+" for international input; digits only, max 15 per E.164. */
  ...
  const handleUseMyData = () => {
    ...
    if (user.phone_number) {
      // Egyptian numbers prefill in the familiar local form; anything else
      // keeps the stored international number as-is.
      const local = parseStoredPhoneToLocal(user.phone_number)
      form.setValue("phone", local ?? sanitizePhoneInput(user.phone_number))
    }
  }
              ...
              <div className="space-y-8">
                ...
                <div className="bg-off-white rounded-2xl p-6 space-y-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span dir="ltr">
                      {watchedPhone
                        ? (phoneToE164(watchedPhone) ?? watchedPhone)
                        : "—"}
                    </span>
                  </div>
                  {selectedTrip.guide_mandatory ? (
