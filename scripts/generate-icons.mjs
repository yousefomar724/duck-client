// Rasterizes public/logo-transparent.png into the PNG icon set the PWA
// manifest and iOS home-screen metadata reference. Re-run with `pnpm icons`
...
const SRC = path.join(ROOT, "public", "logo-transparent.png")
...
async function renderIcon({ size, contentScale, outFile }) {
  ...
  // Trim to the actual ink before scaling — otherwise the logo renders tiny
  // in the middle of the icon instead of filling it.
